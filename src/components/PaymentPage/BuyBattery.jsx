  ...
  return (
    <div id={styles.buybattery}>
      ...
      <div className={styles.options}>
        {options.map((item) => (
          <h3 className={styles.option} style={{color: selectOption === item.text ? '#43655a' : null}} key={item.id}
              onClick={() => setSelectOption(item.text)}>{item.text}</h3>))}
      </div>
      <hr/>
      <div className={styles.aboutBattery}>
        <div className={styles.aboutTitle} style={{fontSize: '20px', fontWeight: "700"}}>배터리란?</div>
        <div className={styles.aboutContent}>챗봇을 구매하기 위해 필요한 전용 결제 수단입니다.</div>
      </div>
